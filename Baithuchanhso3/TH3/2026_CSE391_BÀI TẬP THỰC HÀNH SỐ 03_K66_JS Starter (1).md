...
-Trong console xuất hiện dòng thông báo cho biết JavaScript đã được tải và đang chạy
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
>Khi trang web được tải, trình duyệt sẽ chạy file JavaScript (main.js) và thực thi các lệnh bên trong file đó. JavaScript bắt đầu hoạt động ngay sau khi trang được load.

---

#### Bước 2:  “JavaScript là gì?” (Tra cứu nhanh)

Sử dụng 1–2 nguồn tài liệu (vd. W3Schools, freeCodeCamp, …), tóm tắt:JavaScript là ngôn ngữ lập trình giúp trang web tương tác, động và phản hồi với người dùng.
...
> - JavaScript: dùng để thiết kế giao diện và kiểu dáng cho trang web (màu sắc, font chữ, bố cục…).
...
>   - `number` là: kiểu dữ liệu dùng để biểu diễn các con số trong JavaScript.
>   - `string` là: kiểu dữ liệu dùng để lưu chữ, từ hoặc câu trong JavaScript.
>   - `boolean` là:kiểu dữ liệu dùng để biểu thị giá trị đúng hoặc sai trong JavaScript.
---
...
Xin chào, mình là Linh, năm nay mình 20 tuổi.
-JavaScript đã lấy thông tin name và yearOfBirth mà bạn sửa và in ra console.

> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
Nếu quên dấu ;
→ Thường JavaScript vẫn chạy, vì JS có thể tự thêm dấu ; (automatic semicolon insertion).

Nếu quên dấu + khi nối chuỗi
→ Trình duyệt sẽ báo lỗi trong Console,ví dụ:
...
-Điều thú vị nhất là có thể gõ trực tiếp các lệnh JavaScript trong Console và xem kết quả ngay lập tức mà không cần sửa file hay reload trang web.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
-Em từng quên dấu + khi nối chuỗi, khiến JavaScript báo lỗi trong Console. Em đã đọc thông báo lỗi trong Console và sửa lại cú pháp cho đúng.
...
Cấu trúc if/else giống như các ngã rẽ. Chương trình sẽ kiểm tra từng điều kiện một, nếu điều kiện nào đúng thì đi theo hướng đó và thực hiện lệnh tương ứng. Nếu tất cả điều kiện trên đều sai thì chương trình sẽ đi vào nhánh cuối cùng else.
...
>   - Thân hàm (body): Phần code bên trong { }  dùng để thực hiện các phép tính hoặc xử lý
>   - Giá trị trả về (return): Giá trị được trả về bằng lệnh return, ví dụ: avg hoặc "Giỏi", "Khá"...
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
Ưu điểm của việc dùng hàm là giúp tái sử dụng code nhiều lần, làm chương trình gọn gàng và dễ quản lý hơn, đồng thời tránh phải viết lại cùng một đoạn code nhiều lần.
...
->Phần khiến em khó hiểu nhất là cách hoạt động của if/else khi có nhiều điều kiện và cách hàm trả về giá trị bằng return.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
->Em đã chạy thử nhiều lần trong console, xem lại ví dụ trong bài và tra Google để hiểu rõ hơn cách hoạt động của if/else và hàm.
...
-Lệnh này dùng để tìm và lấy phần tử HTML có id là "status" trong trang web để JavaScript có thể thay đổi nội dung của nó.
> - Sự kiện `"click"` xảy ra khi nào?
-Sự kiện click xảy ra khi người dùng nhấn chuột vào một nút hoặc phần tử trên trang web.
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
-Khi nhấn nút btnHello, nội dung của đoạn văn có id status sẽ thay đổi thành:
...
Sự kiện input xảy ra khi người dùng gõ, sửa hoặc xoá nội dung trong ô nhập liệu.
Còn sự kiện click xảy ra khi người dùng nhấn chuột vào một nút hoặc phần tử trên trang web.
...
>   DOM là mô hình biểu diễn trang HTML như một cây các phần tử, trong đó JavaScript có thể truy cập, thay đổi nội dung hoặc kiểu hiển thị của các phần tử trên trang web.
